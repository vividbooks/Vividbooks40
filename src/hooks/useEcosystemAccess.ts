...
import { isFeatureLicenseActive, isLicenseActive, FeatureLicense, SchoolLicense } from '../types/profile';
  ...
  useEffect(() => {
    const checkAccess = async () => {
      try {
        ...
        // Check if any subject has extended digital access (digital-library tier)
        // This also enables content creation features
        const hasExtendedDigitalAccess = schoolLicense.subjects.some(
          sub => sub.tier === 'digital-library' && isLicenseActive(sub)
        );
        ...
        setAccess({
          ...
          // Ecosystem features - enabled by ecosystem OR extended digital access
          canCreateContent: hasEcosystem || hasExtendedDigitalAccess,
          canUseTeachMe: hasEcosystem || hasExtendedDigitalAccess,
          canUseMyClasses: hasEcosystem,
          // Boards limit
