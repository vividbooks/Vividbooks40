...
function SubjectCard({ subject, license }: { subject: SubjectMeta; license?: SubjectLicense }) {
  ...
  return (
    <div className={`flex-1 rounded-xl p-3 border-2 flex flex-col h-24 min-w-0 ${
      isExtended 
        ? 'bg-indigo-50/50 border-indigo-200' 
        : 'bg-green-50/50 border-green-200'
    }`}>
      <div className={`font-bold text-sm mb-1 truncate ${isExtended ? 'text-indigo-700' : 'text-slate-800'}`}>
        {subject.label}
      </div>
      <span 
      ...
      <div className={`text-[10px] font-medium leading-tight ${
        isExtended ? 'text-indigo-600' : 'text-slate-600'
      }`}>
        {isExtended ? 'Rozšířený' : 'Základní'}
      </div>
