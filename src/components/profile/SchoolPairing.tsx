...
export function SchoolPairing({ onPaired, onClose }: SchoolPairingProps) {
  ...
  const handleSearch = async () => {
    ...
    try {
      // First, try to search in Supabase schools table
      const { data: supabaseSchool, error: supabaseError } = await supabase
        .from('schools')
        .select('*')
        .ilike('code', code)
        .single();

      if (supabaseSchool && !supabaseError) {
        // Found in Supabase
        const school: School = {
        ...
        console.log('✅ School found in Supabase:', school.name);
        setFoundSchool(school);
        return;
      }

      // If not found in Supabase, try localStorage as fallback
      console.log('School not found in Supabase, trying localStorage');
      const localSchool = storage.getSchoolByCode(code);
      if (localSchool) {
        console.log('✅ School found in localStorage:', localSchool.name);
        setFoundSchool(localSchool);
      } else {
        setError('Škola s tímto kódem nebyla nalezena');
      }
    } catch (err) {
      console.error('Error searching school:', err);
