...
import { Sparkles, Plus, Scissors } from 'lucide-react';
...
export function DraggableCanvas({
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      // If click was NOT on a block, NOT on overlay, and NOT on sidebar - deselect
      if (!clickedBlock && !clickedOverlay && !clickedSidebar) {
        onSelectBlock(null);
      }
    };
  ...
  const renderBlocksWithLayout = (pageBlocks: WorksheetBlock[]) => {
    ...
    // Debug: log block widths
    console.log('[Canvas] Rendering blocks:', pageBlocks.map(b => `${b.type}(${b.width || 'undefined'})`).join(', '));
          ...
          <div key={`row-${block.id}`} className="flex items-stretch relative" style={{ minHeight: '40px' }}>
            ...
            >
              ...
              <EditableBlock
                ...
                onUpdate={(content) => onUpdateBlock(nextBlock.id, content)}
                onUpdateMargin={(margin) => onUpdateBlockMargin(nextBlock.id, margin)}
                onDelete={onDeleteBlock ? () => onDeleteBlock(nextBlock.id) : undefined}
                onDuplicate={onDuplicateBlock ? () => onDuplicateBlock(nextBlock.id) : undefined}
                onMoveUp={onMoveBlockUp ? () => onMoveBlockUp(nextBlock.id) : undefined}
                onMoveDown={onMoveBlockDown ? () => onMoveBlockDown(nextBlock.id) : undefined}
                globalFontSize={globalFontSize}
  ...
  return (
    <main 
      ...
      onClick={(e) => {
        ...
        const target = e.target as HTMLElement;
        if (!target.closest('[data-block-id]') && !target.closest('[data-settings-overlay]')) {
          onSelectBlock(null);
        }
      }}
    >
      <div 
        ref={containerRef} 
        className="mx-auto worksheet-pages-row pages-row" 
