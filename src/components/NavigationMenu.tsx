...
export function NavigationMenu({ 
  ...
  const renderMenuItem = (item: MenuItem, depth: number = 0, parentPath: string = '', isInsideWorkbooksTab: boolean = false) => {
    ...
    // Build full path for this item (used for navigation)
    const itemFullPath = item.slug ? (parentPath ? `${parentPath}/${item.slug}` : item.slug) : parentPath;
    ...
    // Build current path for navigation (use item.id as fallback when slug is missing)
    const itemSlugOrId = item.slug || item.id;
    const currentPath = parentPath 
      ? `${parentPath}/${itemSlugOrId}` 
      : itemSlugOrId;
    ...
    const isExternal = ['practice', 'test', 'exam'].includes(item.type || '') && item.externalUrl;
