...
export function MathKeyboard({
  ...
  return (
    <div className="flex flex-col bg-[#E8EAF0] rounded-2xl shadow-xl pt-5" style={{ width: '600px', minWidth: '600px' }}>
      {/* Header with submit button */}
      {onSubmit && (
        <button
          onClick={() => onSubmit(value)}
          disabled={!value.trim()}
          className="mx-5 mb-5 py-4 px-4 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white font-medium text-lg rounded-full transition-colors flex items-center justify-center gap-2"
        >
          Vložit matematický zápis
        </button>
      )}
      
      {/* Preview area with cursor indicator */}
      {showPreview && (
        <div className="bg-white mx-5 mt-5 rounded-xl p-6 min-h-[160px] flex flex-col items-center justify-center border-b-2 border-slate-300">
          {value ? (
            <>
              <div className="text-4xl text-slate-800 mb-4">
                {renderPreview()}
              </div>
              {/* LaTeX source with cursor - MORE VISIBLE */}
              <div className="text-base text-slate-600 font-mono bg-slate-100 px-4 py-2 rounded-lg flex items-center border-2 border-indigo-200">
                <span>{value.slice(0, cursorPosition)}</span>
                <span className="inline-block w-1 h-6 bg-indigo-600 animate-pulse rounded-sm" />
                <span>{value.slice(cursorPosition)}</span>
              </div>
            </>
          ) : (
            <span className="text-slate-400 text-xl">{placeholder}</span>
          )}
        </div>
      )}
      ...
      <div className="flex gap-2 px-5 py-3">
        <button
          ...
          className="flex-1 h-14 rounded-xl flex items-center justify-center transition-colors hover:opacity-90"
      ...
      <div className="px-5 pb-5 space-y-2">
        ...
        <div className="flex gap-2">
          <button onClick={() => handleKeyPress({ label: '7', latex: '7' })} className="flex-1 h-14 rounded-xl text-white text-xl font-medium flex items-center justify-center transition-colors hover:opacity-90" style={{ backgroundColor: '#5C6B7A' }}>7</button>
          <button onClick={() => handleKeyPress({ label: '8', latex: '8' })} className="flex-1 h-14 rounded-xl text-white text-xl font-medium flex items-center justify-center transition-colors hover:opacity-90" style={{ backgroundColor: '#5C6B7A' }}>8</button>
          <button onClick={() => handleKeyPress({ label: '9', latex: '9' })} className="flex-1 h-14 rounded-xl text-white text-xl font-medium flex items-center justify-center transition-colors hover:opacity-90" style={{ backgroundColor: '#5C6B7A' }}>9</button>
          <button onClick={() => handleKeyPress({ label: ':', latex: ':' })} className="flex-1 h-14 rounded-xl text-white text-xl font-medium flex items-center justify-center transition-colors hover:opacity-90" style={{ backgroundColor: '#5C6B7A' }}>:</button>
          <button onClick={() => handleKeyPress({ label: '%', latex: '\\%' })} className="flex-1 h-14 rounded-xl text-slate-600 text-xl flex items-center justify-center transition-colors hover:opacity-80" style={{ backgroundColor: '#CBD5E1' }}>%</button>
        </div>
        
        {/* Row 2 */}
        <div className="flex gap-2">
          <button onClick={() => handleKeyPress({ label: '4', latex: '4' })} className="flex-1 h-14 rounded-xl text-white text-xl font-medium flex items-center justify-center transition-colors hover:opacity-90" style={{ backgroundColor: '#5C6B7A' }}>4</button>
          <button onClick={() => handleKeyPress({ label: '5', latex: '5' })} className="flex-1 h-14 rounded-xl text-white text-xl font-medium flex items-center justify-center transition-colors hover:opacity-90" style={{ backgroundColor: '#5C6B7A' }}>5</button>
          <button onClick={() => handleKeyPress({ label: '6', latex: '6' })} className="flex-1 h-14 rounded-xl text-white text-xl font-medium flex items-center justify-center transition-colors hover:opacity-90" style={{ backgroundColor: '#5C6B7A' }}>6</button>
          <button onClick={() => handleKeyPress({ label: '·', latex: '\\cdot' })} className="flex-1 h-14 rounded-xl text-white text-xl font-medium flex items-center justify-center transition-colors hover:opacity-90" style={{ backgroundColor: '#5C6B7A' }}>·</button>
          <button onClick={() => handleKeyPress({ label: '½', latex: '\\frac{}{}', cursorOffset: 3 })} className="flex-1 h-14 rounded-xl text-slate-600 flex items-center justify-center transition-colors hover:opacity-80" style={{ backgroundColor: '#CBD5E1' }} title="Zlomek"><span className="text-sm"><InlineMath math="\frac{a}{b}" /></span></button>
        </div>
        
        {/* Row 3 */}
        <div className="flex gap-2">
          <button onClick={() => handleKeyPress({ label: '1', latex: '1' })} className="flex-1 h-14 rounded-xl text-white text-xl font-medium flex items-center justify-center transition-colors hover:opacity-90" style={{ backgroundColor: '#5C6B7A' }}>1</button>
          <button onClick={() => handleKeyPress({ label: '2', latex: '2' })} className="flex-1 h-14 rounded-xl text-white text-xl font-medium flex items-center justify-center transition-colors hover:opacity-90" style={{ backgroundColor: '#5C6B7A' }}>2</button>
          <button onClick={() => handleKeyPress({ label: '3', latex: '3' })} className="flex-1 h-14 rounded-xl text-white text-xl font-medium flex items-center justify-center transition-colors hover:opacity-90" style={{ backgroundColor: '#5C6B7A' }}>3</button>
          <button onClick={() => handleKeyPress({ label: '−', latex: '-' })} className="flex-1 h-14 rounded-xl text-white text-xl font-medium flex items-center justify-center transition-colors hover:opacity-90" style={{ backgroundColor: '#5C6B7A' }}>−</button>
          <button onClick={() => handleKeyPress({ label: '√', latex: '\\sqrt{}', cursorOffset: 1 })} className="flex-1 h-14 rounded-xl text-slate-600 text-xl flex items-center justify-center transition-colors hover:opacity-80" style={{ backgroundColor: '#CBD5E1' }}>√</button>
        </div>
        
        {/* Row 4 */}
        <div className="flex gap-2">
          <button onClick={() => handleKeyPress({ label: '0', latex: '0' })} className="flex-1 h-14 rounded-xl text-white text-xl font-medium flex items-center justify-center transition-colors hover:opacity-90" style={{ backgroundColor: '#5C6B7A' }}>0</button>
          <button onClick={() => handleKeyPress({ label: ',', latex: ',' })} className="flex-1 h-14 rounded-xl text-white text-xl font-medium flex items-center justify-center transition-colors hover:opacity-90" style={{ backgroundColor: '#5C6B7A' }}>,</button>
          <button onClick={() => handleKeyPress({ label: '=', latex: '=' })} className="flex-1 h-14 rounded-xl text-white text-xl font-medium flex items-center justify-center transition-colors hover:opacity-90" style={{ backgroundColor: '#5C6B7A' }}>=</button>
          <button onClick={() => handleKeyPress({ label: '+', latex: '+' })} className="flex-1 h-14 rounded-xl text-white text-xl font-medium flex items-center justify-center transition-colors hover:opacity-90" style={{ backgroundColor: '#5C6B7A' }}>+</button>
          <button onClick={() => handleKeyPress({ label: '()', latex: '()', cursorOffset: 1 })} className="flex-1 h-14 rounded-xl text-slate-600 text-lg flex items-center justify-center transition-colors hover:opacity-80" style={{ backgroundColor: '#CBD5E1' }}>( )</button>
        </div>
      </div>
      ...
      {onClose && (
        <div className="px-5 pb-5">
          <button
            onClick={onClose}
            className="w-full py-4 bg-slate-200 hover:bg-slate-300 rounded-xl text-slate-600 font-medium transition-colors text-base"
          >
            Zavřít
          </button>
        </div>
