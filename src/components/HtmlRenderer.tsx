...
export function HtmlRenderer({ content, readerMode = false, hideMethodology = false }: HtmlRendererProps) {
  ...
  useEffect(() => {
    if (containerRef.current && content) {
      ...
      // Simple heuristic: if content contains common Markdown patterns and doesn't look like HTML
      const isMarkdown = (content.includes('##') || content.includes(':::')) && !content.trim().startsWith('<');
