...
export function QuizPreview({ quiz, onClose, isLive = false, onComplete, initialSlideIndex = 0 }: QuizPreviewProps) {
  ...
  return (
    <div className="fixed inset-0 flex z-50" style={{ backgroundColor: bgColor }}>
      ...
      {(showChapterMenu || showNotePanel) && (
        <div className="hidden lg:flex h-full flex-shrink-0">
          {/* Sidebar content */}
          <div className="flex flex-col w-56 flex-shrink-0 bg-white/95 backdrop-blur-sm z-30 border-r border-slate-100">
            <div className="flex-1 overflow-y-auto px-6 py-10" style={{ paddingTop: 100 }}>
              <div className="mb-8">
          ...
          <div className="flex flex-col gap-2 p-4" style={{ paddingTop: 64 }}>
            {showNotePanel ? (
              <>
                {chapters.length > 0 && (
                  <button
                    onClick={() => { setShowNotePanel(false); setShowChapterMenu(true); }}
                    className="w-10 h-10 rounded-full backdrop-blur shadow-sm flex items-center justify-center transition-colors bg-white/80 text-slate-500 hover:bg-white hover:text-slate-700"
                    title="Osnova"
                  >
              ...
              <>
                ...
                {currentSlide?.note && (
                  <button
                    onClick={() => { setShowChapterMenu(false); setShowNotePanel(true); }}
                    className="w-10 h-10 rounded-full backdrop-blur shadow-sm flex items-center justify-center transition-colors bg-white/80 text-slate-500 hover:bg-white hover:text-slate-700"
                    title="Poznámka"
                  >
                    <NoteIcon size={20} />
      ...
      {!showChapterMenu && !showNotePanel && (chapters.length > 0 || currentSlide?.note) && (
