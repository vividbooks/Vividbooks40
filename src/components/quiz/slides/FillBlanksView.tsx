...
export function FillBlanksView({
  ...
  const calculateScore = useCallback(() => {
    ...
    return { correct, total: sentence.blanks.length };
  }, [filledBlanks, correctAnswers, sentence.blanks]);

  const allFilled = Object.keys(filledBlanks).length === sentence.blanks.length;
  ...
  const renderSentence = () => {
    ...
    sortedBlanks.forEach((blank, idx) => {
      ...
      lastIndex = blank.position + blank.text.length;
    });
