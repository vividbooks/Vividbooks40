...
function PairItemEditor({
  ...
  const [imageUrl, setImageUrl] = useState(item.type === 'image' ? item.content : '');
  ...
  const handleImageSave = () => {
    if (imageUrl.trim()) {
      onUpdate({ type: 'image', content: imageUrl.trim() });
    }
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-3 py-1 bg-violet-500 text-white text-xs rounded-lg"
            >
              Uložit
            </button>
            <button
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-violet-500 to-purple-500 p-4">
        ...
        <h2 className="text-white font-bold text-lg">Spoj správné dvojice</h2>
      </div>
