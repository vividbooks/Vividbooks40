...
export function DocumentAIPanel({
        ...
        const response = await fetch(
          `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${GEMINI_API_KEY}`,
          {
            ...
            body: JSON.stringify({
              contents: [{
                role: 'user',
                parts: [{ text: `Převeď tento text do formátu HTML dokumentu. Použij pouze tagy <h2>, <h3>, <p>, <strong>, <em>, <ul>, <li>. NEPOUŽÍVEJ markdown, code tagy ani monospace font. Text:\n\n${combinedContent}` }]
              }]
            })
          }
        );
  ...
  const sendMessage = async (prompt: string) => {
    ...
    try {
      const systemPrompt = `Jsi AI asistent pro tvorbu vzdělávacích dokumentů. Pomáháš učitelům psát a upravovat dokumenty.
...
7. Text musí být v normálním fontu, ne v code/monospace`;
      ...
      const data = await response.json();
      let text = data.candidates?.[0]?.content?.parts?.[0]?.text || 'Nepodařilo se vygenerovat odpověď.';
      ...
      {/* Header */}
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6">
          ...
          <p className="text-sm text-slate-500 text-center mb-6">
            Dokument je prázdný. Vyberte způsob vytvoření obsahu.
          </p>
          ...
          <div className="w-full max-w-xs space-y-3">
            ...
            >
              ...
              <div>
                <p className="font-medium text-slate-800">Vytvořit obsah</p>
                <p className="text-xs text-slate-500">Popsat téma a nechat AI vygenerovat</p>
              </div>
            </button>
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Vytvořit z mých souborů a dokumentů</p>
              </div>
            </button>
          </div>
        </div>
      ...
      >
        {messages.length === 0 ? (
          <div className="space-y-4">
            ...
            {/* Quick Prompts - only show if document has content */}
            {hasContent && (
