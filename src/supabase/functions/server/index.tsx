...
app.post("/make-server-46c8107b/pages", async (c) => {
  try {
    ...
    const { slug, title, content, description, icon, documentType, featuredMedia, sectionImages, parentId, order, category, externalUrl } = await c.req.json();
    ...
    // Check if page with this slug already exists in this category
    const existing = await kv.get(`page:${categoryKey}:${slug}`);
    if (existing) {
      return c.json({ error: "Page with this slug already exists in this category" }, 409);
    }
    ...
    console.log(`Creating page with key: ${key}, title: ${title}, icon: ${icon}`);
    await kv.set(key, page);

    // Update search index
    await updateSearchIndex();

    console.log(`Page created successfully: ${key}`);
...
app.put("/make-server-46c8107b/pages/:slug", async (c) => {
  try {
    ...
    const slug = c.req.param("slug");
    const { title, content, description, icon, documentType, featuredMedia, sectionImages, parentId, order, newSlug, category, externalUrl } = await c.req.json();
        ...
        return items.map(item => {
          if (item.slug === updatedPage.slug) {
            return {
              ...
              icon: updatedPage.icon,
              label: updatedPage.title,
              coverImage: updatedPage.featuredMedia,
